...
async def search_node(search_term: str, influence_filter: bool = Query(False)):
    driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))
    
    influence_types = ["InterpolatesFrom","CoverOf","DirectlySamples","LyricalReferenceTo","InStyleOf"]
    collab_types = ["PerformerOf","ComposerOf","ProducerOf","MemberOf","LyricistOf"]

    with driver.session() as session:
        if influence_filter:
            # This block only runs if influence_filter=true query param is passed
            cypher = """
                MATCH (s:Person)
                WHERE toLower(s.name) = toLower($search_term)
                
                OPTIONAL MATCH (other)-[r]->(s)
                WHERE type(r) IN $influence_types
                
                OPTIONAL MATCH (person:Person)-[role:PerformerOf|ComposerOf|ProducerOf|LyricistOf|MemberOf]->(other)
                
                WITH
                  collect(DISTINCT person) AS persons,
                  collect(DISTINCT role) AS roles,
                  collect(DISTINCT other) AS others,
                  collect(DISTINCT r) AS influence_rels,
                  s
                
                WITH persons + [s] + others AS allNodes, roles + influence_rels AS allRels
